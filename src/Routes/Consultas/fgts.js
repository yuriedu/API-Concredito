  ...
  try {
    ...
    MongoDB.findById('db', async (error, table) => {
      ...
      if (!verify) {
        ...
        if (queue[req.body.bank].length <= 0) return res.status(200).json({ status: false, error: `A fila está vazia! Tente novamente...` })
        if (queue[req.body.bank].findIndex(r=>r.user == req.body.user) < 0) return res.status(200).json({ status: false, error: `Você não está na fila! Tente novamente...` })
